...
class Seisob(object):
    ...
    def __init__(self, authority='local', inventory_object=None, 
        ...
        if not isinstance(inventory_object, obspy.core.inventory.inventory.Inventory):
            self.inventory = None
        else:
    ...
    def _get_picks_arrivals_amplitudes(self, sdf):
        ...
        for ind, row in df4.iterrows(): # iterate through all pick lines
            ...
            picks.append(pick)
            if not pick.phase_hint in specs.amp_phases:
    ...
    def _get_arrival(self, ind, row, pick):
        ...
        ardi['time_weight'] = specs.weight_key.get(row.weight)
    ...
    def _nslc_method_prep(self, sdf):
        ...
        if isinstance(self.inventory, obspy.core.inventory.inventory.Inventory):
            nslcdf = self.wid_df
        else:
            nslcdf = None
        # try using waveform
...
class Sline(object):
    ...
    def read_line(self, sline, validate):
        ...
        if ltype == '4':
            try: # try loading and validating 
                ...
                specs.specs[ltype].validate(self.sseries)
    ...
    def _load_sline(self, sline, ltype):
        ...
        for sp, na, fo in spec:
            ...
            sc = specs.get_string_converter(fo)
...
def _sdf2df_helper(sdf, linetype):    
    ...
    cols = specs.specs[linetype].colname
    df = pd.DataFrame(columns=cols)
