...
def validate_utc(ser, ymd=True, hms=True):
    print("\033[92m****validate_utc****\033[0m ")
    print("ser = "+str(ser))
    ds1 = ['year', 'month', 'day']
    ...
    utc = obspy.UTCDateTime(year=2015, month=1, day=15)
    try:
        if ymd:
            print("\033[92m****utc.year = ser.year****\033[0m ")
            print("\033[92m****ser = "+str(ser)+"****\033[0m ")
            print("\033[92m****ser.year = "+str(ser.year)+"****\033[0m ")
            utc.year = int(ser.year)
            utc.month = int(ser.month)
            utc.day = int(ser.day)
        if hms:
            if ser.hour < 0 or ser.hour > 48:
                raise ValueError('Hour must be between 0 and 48')
            utc.minute = int(ser.minute)
            utc.second = 0
            utc += ser.second
...
def validate_blanks(ser):
    blanks =('bla%d' % d for d in range(1, 12))
    for blank in blanks:
